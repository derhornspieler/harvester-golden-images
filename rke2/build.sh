...
cmd_build() {
  ...
  # Ensure terraform-state namespace exists (backend stores state there)
  ...
  # (happens when state was lost but Harvester resources still exist)
  local state_count
  ...
  if [[ "$state_count" -eq 0 ]]; then
    ...
    [[ -z "$prefix" ]] && prefix="rke2-rocky9-golden"
    local stale=0
    if $KUBECTL get virtualmachineimages.harvesterhci.io "golden-builder-rocky9-base" -n "${vm_namespace}" &>/dev/null; then
      log_warn "Cleaning up orphaned builder image 'golden-builder-rocky9-base'..."
      $KUBECTL delete virtualmachineimages.harvesterhci.io "golden-builder-rocky9-base" -n "${vm_namespace}" --wait=false
      stale=1
    fi
    if $KUBECTL get secret "${prefix}-builder-cloudinit" -n "${vm_namespace}" &>/dev/null; then
      log_warn "Cleaning up orphaned builder secret '${prefix}-builder-cloudinit'..."
    ...
    if $KUBECTL get virtualmachines.kubevirt.io "${prefix}-builder" -n "${vm_namespace}" &>/dev/null; then
      log_warn "Cleaning up orphaned builder VM '${prefix}-builder'..."
