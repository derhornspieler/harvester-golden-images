...
#   3. Removes sensitive files (root-ca.pem, .gitlab-ci.yml, etc.)
...
GITHUB_REMOTE="${GITHUB_REMOTE:-git@github.com:derhornspieler/harvester-golden-images.git}"
BRANCH="${1:-main}"
DRY_RUN=false
...
msg = msg.replace(b"Aegis Group Root CA", b"Private Root CA")
msg = msg.replace(b"Aegis Group", b"Example Org")
msg = msg.replace(b"aegis-root-ca", b"private-ca")
...
msg = msg.replace(b"derhornspieler", b"example-user")
msg = msg.replace(b"hvst-mgmt", b"builder-mgmt")
return msg
' --force 2>/dev/null
...
LEAKS=0
for term in "aegisgroup" "Aegis Group" "frank.jones" "alice.morgan" "derhornspieler" "hvst-mgmt"; do
  COUNT=$(grep -r "${term}" --include="*.yaml" --include="*.yml" --include="*.md" --include="*.sh" --include="*.json" --include="*.tf" --include="*.tpl" --include="*.toml" . 2>/dev/null | grep -v ".git/" | wc -l || true)
  if [[ ${COUNT} -gt 0 ]]; then
...
MSG_LEAKS=$(git log --oneline 2>/dev/null | grep -ciE "aegisgroup|frank\.jones|alice\.morgan|derhornspieler" || true)
